...
/** The copier half of a machine, rendered on the machine's own page.
 *
 * This was its own route until every copier turned out to also be a printer,
 * describing the same connection, model, location and meter in two places.
 * Moved here whole rather than rewritten, so the behaviour an admin relies on
...
export function CopierPanel({
  ...
  useEffect(() => {
    ...
    listMfpDeviceUsage(deviceId, 20)
      .then(setUsage)
      .catch(() => setUsage([]));
    listConnectorTypes()
    ...
    previewMfpDeviceProvisioning(deviceId)
      .then(setPreview)
      .catch(() => setPreview(null));
    getLatestMfpSyncJob(deviceId)
      .then(setJob)
      .catch(() => setJob(null));
    listMfpProvisionedAccounts(deviceId)
      .then(setOwners)
  ...
  async function handleSave() {
    ...
    setActionError(null);
    try {
      const device = await updateMfpDevice(deviceId, {
        ...form,
      ...
      getLatestMfpSyncJob(deviceId)
        .then((latest) => {
          ...
          if (latest && latest.status !== "running" && latest.status !== "pending") {
            listMfpProvisionedAccounts(deviceId).then(setOwners).catch(() => {});
            getMfpDevice(deviceId)
  ...
  async function handleLoadDeviceAccounts() {
    ...
    try {
      setDeviceAccounts(await listMfpDeviceAccounts(deviceId));
    } catch (err) {
  ...
  async function handlePreviewUsers() {
    ...
    setActionError(null);
    try {
      setPreview(await previewMfpDeviceProvisioning(deviceId));
    } catch (err) {
  ...
  async function handleSyncUsers(rewrite = false) {
    ...
    setActionError(null);
    try {
      setJob(await syncMfpDeviceUsers(deviceId, { rewrite }));
    } catch (err) {
  ...
  async function handleAttributeCopies() {
    ...
    setActionError(null);
    try {
      setAttribution(await attributeMfpDeviceCopies(deviceId));
  ...
  async function handleTestConnection() {
    ...
    setActionError(null);
    try {
      const device = await testMfpDeviceConnection(deviceId);
      setState({ phase: "ok", device });
    } catch (err) {
  ...
  async function handleCheckCapabilities() {
    ...
    setActionError(null);
    try {
      const device = await checkMfpDeviceCapabilities(deviceId);
      setState({ phase: "ok", device });
      setCapabilities(device.capabilities);
    } catch (err) {
  ...
  async function handleCheckMeter() {
    ...
    setActionError(null);
    try {
      const device = await checkMfpDeviceMeter(deviceId);
      setState({ phase: "ok", device });
    } catch (err) {
