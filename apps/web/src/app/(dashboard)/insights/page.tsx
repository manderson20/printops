...
function defaultGranularity(preset: DatePreset): ReportGranularity {
  if (preset === "today" || preset === "week" || preset === "month" || preset === "custom") {
    return "day";
  }
  return "week";
}
...
export default function InsightsPage() {
  ...
  const [leaderboardType, setLeaderboardType] = useState<"printer" | "user">("printer");
  ...
  const buildings = useMemo(
    () => [...new Set(printers.map((p) => p.building).filter((b): b is string => !!b))].sort(),
    [printers],
  );
  const departments = useMemo(
    () => [...new Set(printers.map((p) => p.department).filter((d): d is string => !!d))].sort(),
    [printers],
  );
  ...
  const filters: ReportFilters = useMemo(
    ...
    [range, isAdmin, building, department, printerId, submittedBy, statusFilter, colorMode, duplexFilter],
  );
  ...
  const dayOfWeekData = useMemo(() => {
    ...
    return labels.map((label, i) => ({ label, value: state.data.peakTimes.by_day_of_week[i] ?? 0 }));
  }, [state]);
  ...
  return (
    <div className="flex w-full max-w-7xl gap-6">
      <aside className="flex w-64 shrink-0 flex-col gap-4 print:hidden">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-6">
        <div className="flex items-center justify-between print:hidden">
          <div>
            <h1 className="text-xl font-semibold text-black dark:text-zinc-50">Print Insights</h1>
            <p className="mt-1 text-sm text-zinc-500">
              Printing activity, trends, and savings — {isAdmin ? "org-wide" : "your own history"}.
            </p>
          </div>
        ...
        {state.phase === "ok" && (
          <>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              <StatCard label="Total jobs" value={state.data.summary.total_jobs.toLocaleString()} />
              <StatCard label="Total pages" value={state.data.summary.total_pages.toLocaleString()} />
              <StatCard label="Color pages" value={state.data.summary.color_pages.toLocaleString()} />
              <StatCard label="Mono pages" value={state.data.summary.mono_pages.toLocaleString()} />
              <StatCard label="Duplex pages" value={state.data.summary.duplex_pages.toLocaleString()} />
              <StatCard label="Simplex pages" value={state.data.summary.simplex_pages.toLocaleString()} />
              <StatCard
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <Card>
                ...
                <SharePair
                  segments={[
                    { label: "Monochrome", value: state.data.summary.mono_pages, hueName: "blue" },
                    { label: "Color", value: state.data.summary.color_pages, hueName: "aqua" },
                  ]}
                />
              </Card>
              <Card>
                ...
                <SharePair
                  segments={[
                    { label: "Simplex", value: state.data.summary.simplex_pages, hueName: "blue" },
                    { label: "Duplex", value: state.data.summary.duplex_pages, hueName: "aqua" },
                  ]}
                />
              </Card>
            </div>
            ...
            <Card>
              <CardTitle className="mb-3">Environmental &amp; Cost Impact</CardTitle>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                />
                <StatCard label="Trees" value={state.data.summary.trees_used.toFixed(3)} />
                <StatCard label="CO₂" value={`${(state.data.summary.co2_grams / 1000).toFixed(1)} kg`} />
              </div>
...
function SnapshotsSection({ filters, periodLabel }: { filters: ReportFilters; periodLabel: string }) {
  ...
  return (
    <Card className="print:hidden">
      ...
      <p className="mb-3 text-xs text-zinc-500">
        Freezes today&rsquo;s totals and fun facts under a name (e.g. a month or semester) — the saved
        numbers stay fixed even if formulas or job data change later.
      </p>
      <div className="mb-4 flex flex-wrap items-end gap-2">
        <label className="flex flex-col gap-1 text-xs text-zinc-600 dark:text-zinc-400">
          Name
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="March 2026" className="w-40" />
        </label>
        <label className="flex flex-col gap-1 text-xs text-zinc-600 dark:text-zinc-400">
          Start
      ...
      {snapshots !== null && snapshots.length === 0 && <EmptyState>No snapshots saved yet.</EmptyState>}
      {snapshots !== null && snapshots.length > 0 && (
        <div className="flex flex-col gap-2">
          {snapshots.map((snap) => (
            ...
            >
              <div>
                <span className="font-medium text-black dark:text-zinc-50">{snap.name}</span>
                <span className="ml-2 text-xs text-zinc-500">
                  {snap.range_start} – {snap.range_end}
                </span>
                <div className="text-xs text-zinc-500">
                  {snap.totals.total_pages?.toLocaleString?.() ?? snap.totals.total_pages} pages ·{" "}
                  {snap.totals.total_jobs} jobs · saved by {snap.created_by}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge tone="neutral">{new Date(snap.created_at).toLocaleDateString()}</Badge>
                <Button variant="danger" className="!px-3 !py-1 text-xs" onClick={() => handleDelete(snap.id)}>
                  Delete
                </Button>
              </div>
            </div>
...
function ReportFormulasPanel() {
  ...
  useEffect(() => {
    getReportFormulaSettings()
      .then((s) => {
        setSettings(s);
        setForm(Object.fromEntries(FORMULA_FIELDS.map(({ key }) => [key, String(s[key])])));
      })
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Failed to save formulas");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Card className="print:hidden">
      ...
      <p className="mb-3 text-xs text-zinc-500">
        Used to compute the cost/environmental estimates above. Changes only affect future report
        views — saved snapshots keep whatever was calculated at the time.
      </p>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
