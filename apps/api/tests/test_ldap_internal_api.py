...
async def db_session_factory():
    ...
    async with session_factory() as seed:
        seed.add(
            GoogleWorkspaceUser(
                email="matt.anderson@brookfieldr3.org",
                name="Matt Anderson",
                employee_id="1001",
        ...
        seed.add(
            GoogleWorkspaceUser(
                email="jane.smith@brookfieldr3.org",
                name="Jane Smith",
                employee_id="1002",
                synced_at=datetime.now(UTC),
            )
...
def test_search_equality_by_mail(client, auth_headers, backend_headers):
    ...
    assert entries[0]["mail"] == "matt.anderson@brookfieldr3.org"
    assert entries[0]["employee_number"] == "1001"
    assert entries[0]["dn"] == "mail=matt.anderson@brookfieldr3.org,ou=people,dc=printops,dc=local"
...
def test_search_substring_by_cn(client, auth_headers, backend_headers):
    ...
    entries = response.json()["entries"]
    assert len(entries) == 1
    assert entries[0]["mail"] == "jane.smith@brookfieldr3.org"
...
def test_search_uses_configured_base_dn(client, auth_headers, backend_headers):
    update = client.put(
        ...
        json={"enabled": True, "base_dn": "dc=brookfieldr3,dc=org"},
    )
